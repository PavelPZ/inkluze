const qas = [
  inkluze.createQA(
    <span>Asistent pedagoga - u žáka se zdravotním postižením - MP, PAS</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Asistent pedagoga ve školní praxi (konkrétní příklady spolupráce s učitelem)</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Čtenářská a informační gramotnost na ZŠ – 8 hodin</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Akreditace tohoto kurzu již vypršela, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Akreditace kurzu již nebyla prodloužena, protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
    </div>),
  inkluze.createQA(
    <span>Digitální kompetence na základní škole</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Havelková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Digitální kompetence na ZŠ – matematika</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Havelková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Funkce a nástrahy internetu, kyberšikana a sociální sítě</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Akreditace tohoto kurzu již vypršela, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Akreditace kurzu již nebyla prodloužena, protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
    </div>),
  inkluze.createQA(
    <span>GeoGebra jako nástroj rozvoje matematické gramotnosti na ZŠ</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Akreditace tohoto kurzu již vypršela, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Akreditace kurzu již nebyla prodloužena, protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
    </div>),
  inkluze.createQA(
    <span>Jak zvládat stresové situace a nedospět k vyhoření</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Akreditace tohoto kurzu již vypršela, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Akreditace kurzu již nebyla prodloužena, protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
    </div>),
  inkluze.createQA(
    <span>Moderní technologie při rozvoji matematické gramotnosti</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Havelková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Neklidné dítě – problematika ADHD, ADD</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Role pedagoga v karierovém poradenství – 8 hodin</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Akreditace tohoto kurzu již vypršela, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Akreditace kurzu již nebyla prodloužena, protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
    </div>),
  inkluze.createQA(
    <span>Rozvoj podnikavosti a zásady soukromého podnikání</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Akreditace tohoto kurzu již vypršela, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Akreditace kurzu již nebyla prodloužena, protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
    </div>),
  inkluze.createQA(
    <span>Rozvoj sociálních dovedností žáků v ŠD/ŠK</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Forýtková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span> Sebepoškozování (příčiny – řešení – péče o žáka)</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span> Sociální a etický rozvoj – nový předmět na ZŠ</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Forýtková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span> Specifické poruchy chování –pro ŠD a asistenty pedagoga</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Strategie předcházení školní neúspěšnosti </span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Mlátilíková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Strukturované učení u žáků s poruchami autistického spektra</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Školní zralost – příprava předškoláka</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Vilášková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Tandemová výuka</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Forýtková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Třídní učitel a jeho působení na vytváření třídního kolektivu</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Mlátilíková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span> Třídní učitel a wellbeing</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Forýtková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Využití ICT ve výuce - náměty a inspirace pro praxi </span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Havelková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Základy krizové intervence pro pedagogy</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Soukupová pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
  inkluze.createQA(
    <span>Změny v RVP a úpravy ŠVP v rámci programu InspIS ŠVP</span>,
    <div style={{ marginTop: 10 }}>
      <p>
        Tento kurz není akreditován, což není překážkou financování ze Šablon JAK.
      </p>
      <p>
        V případě jakýchkoli pochybností si tuto informaci můžete ověřit na konzultační lince pro Šablony JAK: <a href="mailto:dotazyzp@msmt.cz">dotazyzp@msmt.cz</a>, tel. 234 814 777
      </p>
      <p>
        Kurz byl podán k akreditaci v květnu 2023. Kurz již nebyl zakreditován protože z rozhodnutí MŠMT byly od září 2023 všechny DVPP akreditace zastaveny. Budou se akreditovat MŠMT výhradně specializační a kvalifikační studia, tj. jen ty které vedou ke specializaci či kvalifikaci.
        Více viz: <a href="https://www.msmt.cz/vzdelavani/dalsi-vzdelavani/predpokladane-zmeny-v-akreditacich-vzdelavacich-programu-v" target="_new">(aktualizováno) Změny v akreditacích vzdělávacích programů v systému dalšího vzdělávání pedagogických pracovníků účinné od 1. září 2023, MŠMT ČR (msmt.cz)</a>
      </p>
      <p>
        V bodě 1a) zrušení akreditací - třetí odstavec se píše:
        Zároveň již není akreditace vzdělávacích programů prohlubujícího vzdělávání podmínkou pro čerpání finančních prostředků z dotačních programů, zejména z tzv. šablon Operačního programu Jan Amos Komenský nebo výzev v rámci Národního plánu obnovy.
        </p>
      <p>
        Tento kurz splňuje svou kvalitou kriteria pro akreditovaný kurz. Paní lektorka Forýtková pro nás lektoruje spoustu dalších kurzů, které ji byly bez problémů zakreditovány. Nicméně i jejich akreditace postupně vyprší, stejně tak jako akreditaci všech našich dalších akreditovaných kurzů a i všech kurzů, které nabízejí jiné vzdělavací instituce. Od března 2025 už nebudou akreditované kurzy DVPP vůbec existovat, protože bude všem akreditovaným kurzům ukončena platnost akreditace.
      </p>
    </div>),
]

const QAForm = inkluze.QAForm;

namespace inkluze {
  export const Akreditace: React.StatelessComponent<{}> = (props: {}, ctx: sitemapRouter.IContext) => {
    return <div>
      <Page>
        <Block key={0}>
          <QAForm />
          <h2>Stav akreditací: </h2>
        </Block>
        {qas}
      </Page>
    </div>;
  };

  Akreditace.contextTypes = sitemapRouter.childContextTypes;
}